...
lalonde = pd.read_csv('https://raw.githubusercontent.com/Le-campanella/lalonde_dataset/refs/heads/main/lalonde.csv')
...
def process_data(data: Any, obj: Any, *, backend: str = "auto",
    ...
    raise LMWInputError("Unsupported data type; pass a pandas daraframe or a file path.")
...
def get_X_from_formula(
    ...
    *,
    ...
    # Precompute a "no-treat" RHS using patsy so that both URI and MRI inherit R's
    # term-level reduction semantics (e.g., treat:grp → grp).
    f_no_treat = None
    new_terms: list[str] = []
    interacted_terms: set[str] = set()
    has_intercept = False
    try:
        import patsy as _patsy  # local alias
        f_no_treat, new_terms, interacted_terms, has_intercept = _remove_treat_terms_patsy(
            formula_str, data_proc, treat_name
        )
    except Exception:
        # Fallback: keep original RHS; interactions handled generically later.
        f_no_treat = formula_str
        new_terms = []
        interacted_terms = set()
        has_intercept = True

    # Decide if we use patsy for term-level bookkeeping (required for URI mapping).
    ...
    if use_patsy:
        try:
            ...
            # MRI fallback: build using selected engine with the no-treat formula
            covs_np, cov_colnames = _build_covs_with_engine(f_no_treat, data_proc, treat_name, engine,
                                                            ensure_full_rank, output)
        else:
            # Build covariate design with the no-treat formula via patsy
            Xcov_df = patsy.dmatrix(f_no_treat, data_proc, return_type="dataframe", NA_action="raise")

            # Capture design_info BEFORE any column slicing (pandas may drop attrs on slicing)
            di2 = getattr(Xcov_df, "design_info", None)
            orig_cols_full = list(Xcov_df.columns)  # includes possible 'Intercept'

            cov_cols = [c for c in Xcov_df.columns if c != "Intercept"]

            # --- Enforce R-like treatment contrasts for factors when an intercept is present ---
            from pandas.api.types import CategoricalDtype
            def _cat_baselines(df: pd.DataFrame) -> dict:
                out = {}
            ...
            _baselines = _cat_baselines(data_proc)
            if has_intercept and _baselines:
                def _is_baseline_token(colname: str) -> bool:
                    # Match both patsy-style and plain "[level]" style, on either side of a ":" interaction.
                    for v, b in _baselines.items():
                        b_esc = re.escape(b);
                        v_esc = re.escape(v)
                        # patsy: C(var)[T.level] possibly with extra args inside C(...)
                        pat1 = rf'(^|:)C\({v_esc}(?:,[^)]*)?\)\[T\.{b_esc}\](?=$|:)'
                        # plain: var[level]
                        pat2 = rf'(^|:){v_esc}\[{b_esc}\](?=$|:)'
                        if re.search(pat1, colname) or re.search(pat2, colname):
                            return True
                    return False

                kept = [c for c in cov_cols if not _is_baseline_token(c)]
                if len(kept) != len(cov_cols):
                    cov_cols = kept
                    # Make a sliced copy with the kept columns, preserving order
                    Xcov_df = Xcov_df.loc[:,
                              ["Intercept"] + cov_cols] if "Intercept" in Xcov_df.columns else Xcov_df.loc[:, cov_cols]

            # Final covariate matrix and names (without Intercept)
            covs_np = np.ascontiguousarray(Xcov_df.loc[:, [c for c in Xcov_df.columns if c != "Intercept"]]
                                           .to_numpy(dtype=np.float64, copy=False))
            cov_colnames = [c for c in Xcov_df.columns if c != "Intercept"]

            # Build mapping from reduced terms to current covariate columns.
            # Prefer patsy's term slices if available; otherwise, fall back to robust name matching.
            name_to_new = {name: i for i, name in enumerate(cov_colnames)}

            term_to_indices = {}
            term_slices = getattr(di2, "term_name_slices", {}) if di2 is not None and hasattr(di2,
                                                                                              "term_name_slices") else {}

            for term in new_terms:
                idxs: list[int] = []
                # Try exact term-name lookup via patsy (using the original, pre-slice columns)
                sl = term_slices.get(term) if isinstance(term_slices, dict) else None
                if sl is not None:
                    # Slice into the original full column list
                    term_names_full = orig_cols_full[sl]
                    if not isinstance(term_names_full, (list, tuple)):
                        term_names_full = [term_names_full]
                    for nm in term_names_full:
                        if nm == "Intercept":
                            continue
                        j = name_to_new.get(nm)
                        if j is not None:
                            idxs.append(j)
                # Robust fallback: check whether each kept column belongs to this term
                if not idxs:
                    parts = [p.strip() for p in term.split(":") if p.strip()]

                    def _col_has_symbol(col: str, sym: str) -> bool:
                        sym_esc = re.escape(sym)
                        # numeric or function-like symbols: boundary match on either side of ':'
                        pat_num = rf'(^|:){sym_esc}(?=$|:|\[|\(|,)'
                        # categorical symbols possibly wrapped by C(...)
                        pat_cat = rf'(^|:)C\({sym_esc}(?:,[^)]*)?\)(?=$|:|\[)'
                        pat_plain_cat = rf'(^|:){sym_esc}\[(?:[^\]]+)\](?=$|:)'
                        return bool(re.search(pat_num, col) or re.search(pat_cat, col) or re.search(pat_plain_cat, col))

                    for j, cname in enumerate(cov_colnames):
                        if all(_col_has_symbol(cname, s) for s in parts):
                            idxs.append(j)
                term_to_indices[term] = idxs
    else:
        # No patsy bookkeeping: build covariates with selected engine from the no-treat formula
        covs_np, cov_colnames = _build_covs_with_engine(f_no_treat, data_proc, treat_name, engine,
                                                        ensure_full_rank, output)
    ...
    return XObject(X=X, mf=mf_covs, target=target_payload, names=colnames)
...
def get_w_from_X(X: np.ndarray, treat_contrast: pd.Categorical, method: Method,
    ...
    try:
        ...
        print("scipy QR failed, falling back to np.linalg.pinv")
        XtWX = (X.T * w) @ X
...
def ks_w(x: np.ndarray, treat: pd.Categorical, weights: np.ndarray) -> float:
    ...
    # 与 R 实现一致：在每个唯一取值的“右端点”（该值块的最后一个观测）评估差值
    jumps = np.r_[np.diff(x_ord) != 0, True]
...
def rebuild_Xobject(x: 'LMWResult', data: pd.DataFrame | None = None) -> 'XObject':
    """
    从已有 LMWResult 重建设计矩阵（含列名），镜像 lmw_est() 中的做法。
    方便在外部导出 design matrix，而不用侵入 lmw() 主流程。
    """
    data0 = data if (isinstance(data, pd.DataFrame)) else (x.covs if isinstance(x.covs, pd.DataFrame) else None)
    if data0 is None:
        raise LMWInputError("`data` 必须提供或在 `x.covs` 中可用，用于重建设计矩阵。")
...
def export_design_csv(xobj: 'XObject', path: str = "X_design.csv", include_index: bool = False) -> None:
    """
    把 XObject 的设计矩阵导出为 CSV（带列名）。
    """
    X = np.asarray(xobj.X, dtype=float)
    names = getattr(xobj, "names", None)
    if not names or len(names) != X.shape[1]:
        names = [f"x{j}" for j in range(X.shape[1])]
    pd.DataFrame(X, columns=names).to_csv(path, index=include_index)


def export_weights_csv(obj: 'LMWResult', path: str = "lmw_weights.csv",
                       include_input_weights: bool = True) -> None:
    """
    把 lmw() 的隐含回归权重导出为 CSV，连同处理组、可选输入权重（s.weights/base.weights）。
    """

    n = len(obj.treat)
    t = pd.Series(obj.treat).astype(object).to_numpy()
    w = np.asarray(obj.weights, dtype=float).reshape(-1)
    if w.shape[0] != n:
        raise LMWInputError("weights length does not match number of observations")

    out = pd.DataFrame({
        "row": np.arange(n, dtype=int) + 1,
        "treat": t,
        "weight": w,
    })
    if include_input_weights:
        if obj.s_weights is not None:
            out["s.weight"] = np.asarray(obj.s_weights, dtype=float).reshape(-1)
        if obj.base_weights is not None:
            out["base.weight"] = np.asarray(obj.base_weights, dtype=float).reshape(-1)
    if getattr(obj, "focal", None) is not None:
        out["is_focal"] = (pd.Series(obj.treat).astype(object) == obj.focal).to_numpy()

    out.to_csv(path, index=False)


def export_balance_csv(summary_obj: dict, path: str = "balance_weighted.csv", which: str = "weighted") -> None:
    """
    把 summary_lmw() 的协变量平衡表导出为 CSV。
    which ∈ {'un','base','weighted'} 映射到 {'bal.un','bal.base.weighted','bal.weighted'}。
    """
    key_map = {"un": "bal.un", "base": "bal.base.weighted", "weighted": "bal.weighted"}
    k = key_map.get(str(which).lower(), "bal.weighted")
    tbl = summary_obj.get(k, None)
    if tbl is None:
        raise LMWInputError(f"balance table '{which}' not available in summary")
...
def lmw_est(x: 'LMWResult',
    ...
    # robust_type 可能来自多条路径，统一成大写
    robust_type = robust_type.upper()
    ...
    if cluster_arr is None:
        if robust_type == 'CONST':
            ...
            raise LMWInputError(f"unknown `robust` type: {robust_type}")
    else:
    ...
    return LMWEstResult(
        ...
        coef_names=getattr(X_obj, "names", None),
    )
...
# ----------------------------
# Summary for estimation results – Python port of R/summary.lmw_est
# ----------------------------

def _zcrit_from_level(level: float) -> float:
    """Return z critical value for two-sided normal CI at given confidence level.
    Uses a small lookup for common levels; defaults to 0.95.
    """
    table = {
    ...
    return float(table.get(round(float(level), 3), table[0.95]))

def summary_lmw_est(obj: LMWEstResult,
                    confint: bool = True,
                    level: float = 0.95,
                    include_intercept: bool = True) -> dict:
    """Create a structured summary for an `LMWEstResult`.

    Returns a dict with entries similar to R's summary.lmw_est():
    - coef_table: pandas DataFrame with columns [coef, se, z, p, (lwr), (upr)]
    - r.squared, adj.r.squared, sigma (resid. std. error)
    - df.residual, nobs, rank, robust, estimand, method
    """
    import numpy as _np
    import pandas as _pd
    from math import erf, sqrt

    coef = _np.asarray(obj.coefficients, dtype=float)
    se = _np.sqrt(_np.maximum(_np.diag(obj.vcov), 0.0))
    z = coef / _np.where(se > 0, se, _np.nan)
    p = 2.0 * (1.0 - 0.5 * (1 + _np.vectorize(lambda v: erf(abs(v) / sqrt(2)))(z)))

    names = obj.coef_names if getattr(obj, 'coef_names', None) else [f"x{j}" for j in range(coef.shape[0])]
    coef_df = _pd.DataFrame({"coef": coef, "se": se, "z": z, "p": p}, index=names)

    if confint:
        zc = _zcrit_from_level(level)
        coef_df["lwr"] = coef_df["coef"] - zc * coef_df["se"]
        coef_df["upr"] = coef_df["coef"] + zc * coef_df["se"]

    if not include_intercept and "(Intercept)" in coef_df.index:
        coef_df = coef_df.drop(index=["(Intercept)"])

    # Weighted R² and related metrics
    y = _np.asarray(obj.residuals + obj.fitted_values, dtype=float)
    w = _np.asarray(obj.weights, dtype=float)
    pos = (w > 0)
    nobs = int(pos.sum())
    sw = float(w[pos].sum()) if nobs else 0.0
    sse = float(_np.sum(w[pos] * (obj.residuals[pos] ** 2))) if nobs else _np.nan
    if nobs and sw > 0.0:
        ybar = float(_np.sum(w[pos] * y[pos]) / sw)
        tss = float(_np.sum(w[pos] * ((y[pos] - ybar) ** 2)))
        r2 = float(1.0 - sse / tss) if tss > 0 else _np.nan
    else:
        r2 = _np.nan
    df_resid = int(getattr(obj, 'df_residual', _np.nan))
    p_rank = int(getattr(obj, 'rank', coef.shape[0]))
    adj_r2 = float(1.0 - (1.0 - r2) * (max(nobs - 1, 1) / max(df_resid, 1))) if _np.isfinite(r2) else _np.nan
    sigma = float(_np.sqrt(sse / max(df_resid, 1))) if (nobs and _np.isfinite(sse)) else _np.nan
    ...
    return {
        ...
        "nobs": nobs,
        "rank": p_rank,
        "robust": getattr(obj, 'robust', None),
        "estimand": getattr(obj, 'estimand', None),
        "method": getattr(obj, 'method', None),
        "outcome": getattr(obj, 'outcome', None),
    }

def print_summary_lmw_est(s: dict, digits: int = 3) -> None:
    import pandas as _pd
    def _fmt_df(df: _pd.DataFrame) -> _pd.DataFrame:
        out = df.copy()
        for c in out.columns:
            out[c] = out[c].apply(lambda v: (f"{round(v, digits):.{digits}f}" if isinstance(v, (int, float)) else v))
        return out
    coef_tbl = s.get("coefficients", None)
    if coef_tbl is not None:
        print("\nCoefficients:")
        print(_fmt_df(coef_tbl))
    meta = [
        ("R-squared", s.get("r.squared", None)),
        ("Adj. R-squared", s.get("adj.r.squared", None)),
        ("Sigma", s.get("sigma", None)),
        ("DF Residual", s.get("df.residual", None)),
        ("Nobs", s.get("nobs", None)),
        ("Rank", s.get("rank", None)),
        ("SE type", s.get("robust", None)),
        ("Estimand", s.get("estimand", None)),
        ("Method", s.get("method", None)),
        ("Outcome", s.get("outcome", None)),
    ]
    print("\nModel summary:")
    for k, v in meta:
        if v is not None:
...
def _kde1d(x: np.ndarray,
           grid_n: int = 512,
           bw: float | None = None,
           adjust: float | None = None,
           cut: float = 3.0) -> tuple[np.ndarray, np.ndarray]:
    """Lightweight Gaussian KDE mimicking R's stats::density defaults.

    Parameters
    ----------
    x : array-like
        Sample values (weights etc.), finite values only considered.
    grid_n : int
        Number of evaluation points.
    bw : float or None
        Bandwidth. If None, use R's bw.nrd0: 0.9 * min(sd, IQR/1.34) * n**(-1/5).
    adjust : float or None
        If provided, multiply the bandwidth by this factor (like R's `adjust`).
    cut : float
        Extend the evaluation range by `cut * bw` on both sides (R default = 3).

    Returns
    -------
    (xs, ys) : tuple[np.ndarray, np.ndarray]
        Grid and density values. The integral of ys over xs is ~1.
    """
    x = np.asarray(x, dtype=float)
    x = x[np.isfinite(x)]
    if x.size == 0:
    ...
    # R's bw.nrd0
    if bw is None:
        q75, q25 = np.percentile(x, [75, 25]) if n > 1 else (mu, mu)
        iqr = float(q75 - q25)
        scale = min(sd, iqr / 1.34) if (sd > 0 or iqr > 0) else 0.0
        if scale <= np.finfo(float).eps:
            # near-constant fallback
            bw = max(abs(mu) * 1e-2, 1e-3)
        else:
            bw = 0.9 * scale * (n ** (-1.0 / 5.0))

    if adjust is not None:
        try:
            bw = float(bw) * float(adjust)
        except Exception:
            pass

    h = float(bw) if bw and np.isfinite(bw) and bw > 0 else max(abs(mu) * 1e-2, 1e-3)

    # Evaluation grid like R's `cut`
    left = float(x.min()) - cut * h
    right = float(x.max()) + cut * h
    ...
    xs = np.linspace(left, right, int(grid_n))

    # Gaussian kernels
    diffs = (xs[:, None] - x[None, :]) / h
    ys = np.exp(-0.5 * diffs * diffs).sum(axis=1) / (n * h * np.sqrt(2*np.pi))
...
def plot_lmw(x: 'LMWResult', type: str = "weights", save: str | None = None, dpi: int | None = 300, **kwargs) -> 'LMWResult':
    """Matplotlib port of R's plot.lmw().
    type in {'weights','extrapolation','influence'}.
      - weights: rug=True, mean=True, ess=True, bw=None, adjust=None, cut=3.0
      - extrapolation: variables=..., data=None
      - influence: outcome=None, data=None, id_n=3  (currently NotImplemented)
    Parameters
    ----------
    x : LMWResult
        The result object to plot.
    type : str
        Plot type: 'weights', 'extrapolation', or 'influence'.
    save : str or None
        If provided, save the plot to this path (e.g., "weights.png" / "extrapolation.pdf").
    dpi : int or None
        Dots per inch when saving raster formats; ignored for vector formats.
    """
    t = str(type).lower()
    if t not in {"weights", "extrapolation", "influence"}:
        raise LMWInputError("type must be one of 'weights', 'extrapolation', 'influence'")
    if t == "weights":
        return _plot_weights_lmw(x, save=save, dpi=dpi, **kwargs)
    elif t == "extrapolation":
        return _plot_extrapolation_lmw(x, save=save, dpi=dpi, **kwargs)
    else:
        return _plot_influence_lmw(x, save=save, dpi=dpi, **kwargs)

def _plot_weights_lmw(x: 'LMWResult',
                      rug: bool = True,
                      mean: bool = True,
                      ess: bool = True,
                      bw: float | None = None,
    ...
    for row, i in enumerate(tlevs):
        ...
        xs, ys = _kde1d(wi, bw=bw, adjust=adjust, cut=cut)
...
def _plot_extrapolation_lmw(x: 'LMWResult', variables: Any, data: pd.DataFrame | None = None, save: str | None = None, dpi: int | None = 300, **kwargs) -> 'LMWResult':
    import matplotlib.pyplot as plt
...
def plot_summary_lmw(x: dict,
    ...
    save : str or None
        If provided, save the figure to this path (e.g., "love_plot.png").
    dpi : int or None
        Dots per inch when saving raster formats; ignored for vector formats.
    ...
    for ax, stat_name, xlabel in zip(axes, chosen, chosen_labels):
        # Insert color/legend configuration and color logic
        from matplotlib.lines import Line2D
        c_un, c_base, c_w = 'C0', 'C1', 'C2'
        ...
        if stats_un is not None:
            ax.plot(stats_un[stat_name].to_numpy()[ord_idx], y_pos, linestyle='None', marker='x', markersize=5, color=c_un)
        if stats_base is not None:
            ax.plot(stats_base[stat_name].to_numpy()[ord_idx], y_pos, linestyle='None', marker='o', markersize=6, fillstyle='none', color=c_base)
        if stats_w is not None:
            ax.plot(stats_w[stat_name].to_numpy()[ord_idx], y_pos, linestyle='None', marker='o', markersize=6, color=c_w)
        ...
        # Legend (explicit handles and labels, matching marker/color to plot order)
        handles = []
        labels = []
        if stats_un is not None:
            handles.append(Line2D([0], [0], linestyle='None', marker='x', markersize=6, color=c_un))
            labels.append(legend_text[len(labels)])
        if stats_base is not None:
            handles.append(Line2D([0], [0], linestyle='None', marker='o', markersize=7, fillstyle='none', color=c_base))
            labels.append(legend_text[len(labels)])
        if stats_w is not None:
            handles.append(Line2D([0], [0], linestyle='None', marker='o', markersize=7, color=c_w))
            labels.append(legend_text[len(labels)])
...
# ----------------------------
# Plotting for estimation results – Python port of R/plot.lmw_est(type="lm")
# ----------------------------

def plot_lmw_est(obj: LMWEstResult,
                 type: str = "lm",
                 level: float = 0.95,
                 include_intercept: bool = False,
                 order: str = "abs",  # ignored for diagnostics
                 top_n: int | None = None,  # ignored for diagnostics
                 threshold: float | None = None,  # ignored for diagnostics
                 save: str | None = None,
                 dpi: int | None = 300) -> LMWEstResult:
    """Diagnostic plots for an `LMWEstResult`, emulating R's `plot.lm()`.

    When ``type='lm'`` this function now draws **four core diagnostic charts**:
      1) Residuals vs Fitted (with a simple smooth/line)
      2) Normal Q–Q of standardized residuals
      3) Scale–Location: sqrt(|standardized residuals|) vs Fitted
      4) Residuals vs Leverage with Cook's distance contours (0.5 and 1)

    Notes
    -----
    - Any coefficient-plot-related arguments (``order``, ``top_n``, ``threshold``,
      ``include_intercept``) are accepted but **ignored** for diagnostics to preserve
      backward compatibility.
    - For weighted fits we follow the base-R recipe: compute leverage on
      ``A = sqrt(weights) * X``; standardized residuals use ``sigma`` and ``1-h``.
    """
    import warnings
    import numpy as _np
    ...
    t = str(type).lower()
    if t != "lm":
        raise LMWInputError("plot_lmw_est currently supports only type='lm' (diagnostics)")

    # --- Extract arrays ---
    X = _np.asarray(obj.model_matrix, dtype=float)
    w = _np.asarray(obj.weights, dtype=float).reshape(-1)
    resid = _np.asarray(obj.residuals, dtype=float).reshape(-1)
    fitted = _np.asarray(obj.fitted_values, dtype=float).reshape(-1)

    if X.shape[0] != w.shape[0] or resid.shape[0] != w.shape[0]:
        raise LMWInputError("Dimensions of model matrix / residuals / weights do not agree.")

    pos = (w > 0)
    if not _np.any(pos):
        raise LMWInputError("No positive regression weights to plot diagnostics.")

    # Subset to positive weights (matches R's behavior in weighted fits)
    Xp = X[pos, :]
    wp = w[pos]
    rp = resid[pos]
    fp = fitted[pos]

    # Hat values on A = sqrt(w) * X
    Ap = _np.sqrt(wp)[:, None] * Xp
    h = _hat_diag_from_A(Ap)

    # Sigma and standardized residuals rstandard = e / (sigma * sqrt(1 - h))
    df_resid = int(getattr(obj, 'df_residual', max(Xp.shape[0] - Xp.shape[1], 1)))
    sigma2 = float((_np.sum(wp * (rp ** 2))) / max(df_resid, 1))
    sigma = float(_np.sqrt(max(sigma2, _np.finfo(float).tiny)))
    denom = _np.sqrt(_np.maximum(1.0 - _np.clip(h, 0.0, 1.0 - 1e-12), _np.finfo(float).tiny))
    # Weighted standardized residuals (align with rstandard.lm for WLS):
    # rstd = e_i * sqrt(w_i) / (sigma * sqrt(1 - h_ii))
    rstd = (rp * _np.sqrt(wp)) / (sigma * denom)

    # Cook's distance; use studentized-like formula D = rstd^2 * h / p
    p = int(getattr(obj, 'rank', Xp.shape[1]))
    cooks = (rstd ** 2) * _np.clip(h, 1e-12, 1.0) / max(p, 1)

    # Helper: simple linear smoother (fallback for loess)
    def _smooth_xy(x, y):
        try:
            # Use a simple LOWESS if statsmodels is available
            import statsmodels.api as sm  # type: ignore
            z = sm.nonparametric.lowess(y, x, frac=0.3, return_sorted=True)
            return z[:, 0], z[:, 1]
        except Exception:
            # Fallback: first-degree polynomial fit
            x0 = _np.asarray(x)
            y0 = _np.asarray(y)
            if x0.size < 2:
                return x0, y0
            ...
            x1 = x0[idx]
            y1 = y0[idx]
            b = _np.polyfit(x1, y1, deg=1)
            yhat = _np.polyval(b, x1)
            return x1, yhat

    # Helper: inverse CDF of standard normal (Acklam approximation)
    def _norm_ppf(p):
        ...
        mask = p > phigh
        if _np.any(mask):
            q = _np.sqrt(-2.0 * _np.log(1.0 - p[mask]))
            x[mask] = -(((((c[0]*q + c[1])*q + c[2])*q + c[3])*q + c[4])*q + c[5]) / \
                       ((((d[0]*q + d[1])*q + d[2])*q + d[3])*q + 1.0)
        return x
    ...
    ax = axes[0, 0]
    ax.scatter(fp, rstd, s=12, facecolors='none', edgecolors='k')
    xs, ys = _smooth_xy(fp, rstd)
    ...
    # (2) Normal Q–Q
    ax = axes[0, 1]
    r_sorted = _np.sort(rstd)
    n = r_sorted.size
    probs = ( _np.arange(1, n + 1) - 0.5 ) / n
    probs = _np.clip(probs, 1e-12, 1.0 - 1e-12)
    theo = _norm_ppf(probs)
    ax.plot(theo, r_sorted, 'o', markersize=3)
    # Reference line through Q1/Q3
    rq = _np.quantile(r_sorted, [0.25, 0.75])
    tq = _np.quantile(theo, [0.25, 0.75])
    slope = (rq[1] - rq[0]) / max(tq[1] - tq[0], _np.finfo(float).eps)
    intercept = rq[0] - slope * tq[0]
    lo, hi = theo.min(), theo.max()
    ax.plot([lo, hi], [intercept + slope*lo, intercept + slope*hi], color='red', linewidth=1)
    ...
    # (3) Scale–Location
    ax = axes[1, 0]
    y_sc = _np.sqrt(_np.abs(rstd))
    ax.scatter(fp, y_sc, s=12, facecolors='none', edgecolors='k')
    xs, ys = _smooth_xy(fp, y_sc)
    ax.plot(xs, ys, color='red', linewidth=1)
    ax.set_xlabel("Fitted values")
    ...
    ax = axes[1, 1]
    ax.scatter(h, rstd, s=12, facecolors='none', edgecolors='k')
    ax.axhline(0.0, color='gray', linestyle='--', linewidth=0.8)
    ax.set_xlabel("Leverage")
    ax.set_ylabel("Standardized residuals")
    ax.set_title("Residuals vs Leverage")

    # —— 用散点确定坐标范围（Cook 曲线不应主导缩放）——
    x_min, x_max = float(_np.min(h)), float(_np.max(h))
    x_pad = 0.05 * max(x_max - x_min, 1e-8)
    x_left = max(0.0, x_min - x_pad)
    x_right = min(0.999, x_max + x_pad)

    y_min, y_max = float(_np.min(rstd)), float(_np.max(rstd))
    y_pad = 0.10 * max(y_max - y_min, 1e-8)
    y_bottom, y_top = y_min - y_pad, y_max + y_pad

    ax.set_xlim(x_left, x_right)
    ax.set_ylim(y_bottom, y_top)
    ax.set_autoscale_on(False)  # 锁定范围，防止 Cook 曲线改变缩放

    # —— Cook's distance 等值线（0.5 与 1），仅在当前 x 范围内绘制 ——
    s = _np.linspace(max(1e-6, x_left), min(0.999, x_right), 200)
    for c, lab in [(0.5, "0.5"), (1.0, "1")]:
        with _np.errstate(divide='ignore', invalid='ignore'):
            y = _np.sqrt(c * max(p, 1) * (1.0 - s) / _np.clip(s, 1e-12, None))
        ax.plot(s, y, linestyle='--', color='gray')
        ax.plot(s, -y, linestyle='--', color='gray')
        # 标签放在面板右侧 90% 处且保证在轴内
        xlab = x_left + 0.9 * (x_right - x_left)
        ylab = _np.sqrt(c * max(p, 1) * (1.0 - xlab) / max(xlab, 1e-12))
        if _np.isfinite(ylab):
            ylab = _np.clip(ylab, y_bottom + 0.02 * (y_top - y_bottom), y_top - 0.02 * (y_top - y_bottom))
            ax.text(xlab, ylab, f"{lab}", color='gray', fontsize=8)
            ax.text(xlab, -ylab, f"{lab}", color='gray', fontsize=8)
    ...
    # Optionally label the largest Cook's distances (top 3 by default)
    id_n_eff = 3 if (top_n is None or top_n <= 0) else int(top_n)
    if id_n_eff > 0 and cooks.size:
        idx_top = _np.argsort(cooks)[-id_n_eff:]
        idx_top = idx_top[_np.argsort(h[idx_top])]  # left to right
        # map back to original row numbers
        orig_idx = _np.flatnonzero(pos)
        for i in idx_top:
            ax.text(h[i], rstd[i], str(int(orig_idx[i] + 1)), fontsize=8,
                    ha='left', va='bottom')
...
def main():
    # -------------------------------
    # 1) Run lmw design (weights)
    # -------------------------------
    df = pd.read_csv('https://raw.githubusercontent.com/Le-campanella/lalonde_dataset/refs/heads/main/lalonde.csv')
    formula = "~ treat + age + education + race + married + nodegree + re74 + re75 + treat:re74 + treat:re75"
    ...
    # summ = summary_lmw(res, un=True, stat="balance", data=df)
    # print_summary(summ)

    # -------------------------------
    # 2) Balance summary + Love plot
    # -------------------------------

    # os.makedirs("plots", exist_ok=True)
    # plot_summary_lmw(
    #     summ,
    #     stats=["TSMD Treated", "TSMD Control", "KS"],
    #     abs=True,
    #     var_order="unadjusted",
    #     threshold=[0.1, 0.25],
    #     layout="vertical",
    #     save=os.path.join("plots", "love_plot_URI_ATE.png"),
    #     dpi=300,
    # )

    # -------------------------------
    # 3) Design/weight diagnostics
    # -------------------------------
    # plot_lmw(
    #     res,
    #     type="weights",
    ...
    #     mean=True,
    #     ess=True,
    #     save=os.path.join("plots", "weights_URI_ATE.png"),
    #     dpi=300,
    # )
    #
    # plot_lmw(
    #     res,
    #     type="extrapolation",
    #     variables="~ age + education + re74",
    #     data=df,
    #     save=os.path.join("plots", "extrapolation_URI_ATE.png"),
    # )
    #
    # plot_lmw(
    #     res,
    ...
    #     data=df,
    #     id_n=5,
    #     save=os.path.join("plots", "influence_URI_ATE.png"),
    # )
    #
    # # -------------------------------
    # # 4) Estimation + coefficient plot
    # # -------------------------------
    est = lmw_est(res, outcome="re78", data=df, robust="HC3")
    s_est = summary_lmw_est(est, confint=True, level=0.95, include_intercept=False)
    print_summary_lmw_est(s_est)
    ...
    plot_lmw_est(
        ...
        order="abs",
        top_n=25,
        save=os.path.join("plots", "coef_forest_URI_ATE.png"),
        dpi=300,
    )
    #
    # print("\nSaved figures to ./plots :\n - love_plot_URI_ATE.png\n - weights_URI_ATE.png\n - extrapolation_URI_ATE.pdf\n - influence_URI_ATE.svg\n - coef_forest_URI_ATE.png")
